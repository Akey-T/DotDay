...
|------|----------|-------|--------|--------|
| 2026-08-12 | Codex | `src/shared/types.ts`, `src/main/index.ts`, `src/preload/index.ts`, `src/renderer/src/main.tsx`, `src/renderer/src/components/InsightsPanel.tsx`, `src/renderer/src/utils/analytics.ts`, `src/renderer/src/styles.css` | Added habit editing, direct progress value entry, incomplete-first Today Habits ordering, Settings data backup/import/open-folder actions, optional native event notifications, and richer Insights summaries. | Makes daily use faster, backup safer, reminders more flexible, and analytics more actionable without changing DotDay's compact widget shape. |
| 2026-06-29 | Codex | `src/shared/types.ts`, `src/main/index.ts`, `src/renderer/src/main.tsx`, `src/renderer/src/utils/analytics.ts`, `src/renderer/src/components/InsightsPanel.tsx`, `src/renderer/src/styles.css` | Added progress-based habits with target/unit settings, plus/minus daily adjustment, proportional completion scoring, and compatible analytics across Today, Calendar, Insights, streaks, and per-habit history. | Supports habits like drinking 5 cups of water while keeping checkbox habits simple and making all completion averages exclude future/no-data days with consistent per-day weighting. |
| 2026-06-29 | Codex | `src/shared/types.ts`, `src/main/index.ts`, `src/renderer/src/main.tsx`, `src/renderer/src/utils/analytics.ts` | Added per-day progress habit updated timestamps and displayed the latest update time beside progress labels. | Helps users notice duplicate or mistaken progress entries without adding a separate activity log. |
| 2026-06-25 | Codex | `src/renderer/src/styles.css` | Fixed Quick Notes content being compressed by limiting flexible width to the card's content container instead of its action buttons. | Restores the full available text width while keeping edit and delete controls compact on the right. |
| 2026-06-25 | Codex | `src/renderer/src/main.tsx` | Added outside-click dismissal to Settings while keeping controls inside the panel interactive. | Matches the convenient dismissal behavior used by Calendar and Insights. |
| 2026-06-25 | Codex | `src/renderer/src/main.tsx` | Added layered outside-click dismissal for Calendar and Insights. | Clicking the backdrop now closes Insights first when open, then closes Calendar from its normal view, while preserving all interactions inside either panel. |
| 2026-06-25 | Codex | `src/main/index.ts` | Enabled Launch at startup while running DotDay from PowerShell by registering Electron with the project path on Windows; packaged builds continue to register their own executable. | Makes the setting testable and functional in development without creating a broken bare-Electron startup entry. |
| 2026-06-25 | Codex | `src/shared/types.ts`, `src/main/index.ts`, `src/renderer/src/main.tsx`, `src/renderer/src/styles.css`, `README.md` | Added Moment, cross-day Duration, and All-day event modes; added a half-page Plan Notes checklist with editing, deletion, completion, importance, and drag ordering; collapsed the Quick Notes input behind its add button. | Makes event capture fit real-world points and ranges while adding a lightweight undated planning space without crowding the main dashboard. |
